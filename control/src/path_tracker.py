...
def tracker(msg:PoseStamped):
    ...
    if not done_once:
        ...
        while math.sqrt(inc_x**2 + inc_y**2) > 0.08:
            if twist.linear.x < 0.6 and math.sqrt(inc_x**2 + inc_y**2)>twist.linear.x**2/(2*acceleration):
                twist.linear.x += acceleration
                rospy.loginfo(twist.linear.x)
                # pub_twist.publish(twist)
                # rate.sleep()
                
            elif twist.linear.x >= 0.6 and math.sqrt(inc_x**2 + inc_y**2)>twist.linear.x**2/(2*acceleration): #eller acceleration
                twist.linear.x = 0.6
                # pub_twist.publish(twist)
                # rate.sleep()
