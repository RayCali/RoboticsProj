...
# https://answers.ros.org/question/113671/catkin-package-cannot-find-own-message-type-python/
...
CENTER_IMAGE_X = 640 # center of image in x direction
CENTER_IMAGE_Y = 360 # center of image in y direction

def imageCB(msg):
    
    # msg is of type Image, convert to torch tensor
    image = rnp.numpify(msg) # shape: (720,1280,3)
    image = transforms.ToTensor()(image)
    image = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])(image)
    image = image.unsqueeze(0)
    
    bbs = detectionModel.decode_output(inference, threshold=0.7)[0]
    loginfo("Bounding boxes: {}".format(bbs))
    ...
    for bb in bbs:
        ...
        error_x = centerbb_x - CENTER_IMAGE_X
...
if __name__=="__main__":
    ...
    loginfo("Starting detection node")
    detectionModel = utils.load_model(detector.Detector(),"/home/robot/models/working_model/index.pt", device="cpu")
    detectionModel.eval()
    
    image = pil.open("/home/robot/Downloads/frame0008.jpg")

    image = transforms.ToTensor()(image)
    image = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])(image)
    image = image.unsqueeze(0)
    inference = detectionModel(image)
